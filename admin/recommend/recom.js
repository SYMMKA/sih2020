					...
					for (sem in data[branch]) {
						html +=
                            ...
                            <div
                                ...
                                href
                            >
                                <h1 class="card-title sem">` + sem + `</h1>
                            </div>
                        </div>`;
					}
					html += `</div>
...
branchDrop.onchange = function () {
	...
	$('#recommendations .show-books').filter(function () {
		$(this).toggle(
			$(this).find('.sem').text().indexOf('') > -1
		);
	});
	// filter branch wise
	$('#recommendations .sem_branch').filter(function () {
		$(this).toggle(
			$(this).find('.branch').text().indexOf(branchVal) > -1);
	});
...
semesterDrop.onchange = function () {
	...
	$('#recommendations .show-books').filter(function () {
		$(this).toggle(
			$(this).find('.sem').text().indexOf(semVal) > -1
		);
	});
};
