function autoFillInfo(i) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            data.forEach((element) => {
                if (element.status == "issued") issued++;
                else if (
    ...
    else document.getElementById("bookBookInfo").textContent = "Audio";
    if (digital[i] == 1)
        document.getElementById("bookDigitalInfo").textContent = "Digital";
    else document.getElementById("bookDigitalInfo").textContent = "Physical";
    if (imgLink[i]) {
    ...
    else document.getElementById("groupPagesInfo").hidden = true;
