...
try {
	...
	$sqlRequestBook = "DELETE FROM `request_list` WHERE `title` = :title AND `author` = :author AND `isbn` = :isbn";
	$stmt3RequestBook = $conn->prepare($sqlRequestBook);
	$stmt3RequestBook->bindParam(':title', $title2);
	$stmt3RequestBook->bindParam(':author', $author2);
	$stmt3RequestBook->bindParam(':isbn', $isbn2);
	$stmt3RequestBook->execute();
